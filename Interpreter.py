...
class Interpreter(Generic[VI]):
    ...
    def __init__(self):
        ...
        self.output_list = collections.OrderedDict()
        ...
        while True:
            ...
            for line in lines:
                if line[0] == '>':
                    ...
                    lt = line.strip("\n").split(":")
                    at = lt[0].split("-->")
                    if len(lt) != 2 or len(at) != 4:
                        logger.error("Input invalid.")
                        return False
                    else:

                        self.add_trans(at[0], at[1], at[2], int(at[3]), int(lt[1]))
        return True
        ...
        for t in self.trans_list:
            ...
            for s in self.input_list.keys():
                if self.input_list[s] == t[1]:
                    inrt = s
            for s in self.output_list.keys():
                if self.output_list[s] == t[3]:
                    outrt = s
            self.action_table[inct][inrt] = [outt, outrt, t[4]]
    ...
    def add_output(self, output) -> None:
        """
        Add a new input signal to the interpreter
        :param output: new output signal
        :return: None
        """
        self.output_list[self.output_count] = output
        self.output_count += 1

    @typecheck(object, str, str, str, int)
    def add_trans(self, ins: str, inc: str, outs: str, outc: str, times: int) -> None:
        """
        Add a new state transition to the interpreter
        :param ins: Current state
        :param inc: input signal
        :param outs: transferred state
        :param outc: output signal
        ...
        if inc not in self.input_list.values():
            ...
            self.input_count += 1
        if outc not in self.output_list.values():
            self.output_list[self.output_count] = outc
            self.output_count += 1
        self.trans_list.append([ins, inc, outs, outc, times])
        ...
        while self.count < len(self.input_queue):
            if timer >= max_t and next_clock == self.clock:
                ...
                logger.info("transition: at clock " +
                            ...
                            str(self.input_list[index]) +
                            "\t,\toutput : " +
                            str(self.output_list[next[1]]) +
                            "\t,\tnext_state : " +
                            str(self.state_list[next[0]]))
                record = [self.clock, self.state, str(self.input_list[index]), str(self.output_list[next[1]]), str(self.state_list[next[0]])]
                self.history.append(record)
